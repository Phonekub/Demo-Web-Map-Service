...
export const DeliveryArea: React.FC<TradeAreaProps> = ({ poiId, location }) => {
  ...
  const tradeareaType = 'delivery_area';
  ...
  // Extract center coordinates from location — try multiple fields
  const getLocationCoordinates = (): number[] => {
    if (location?.geom?.coordinates?.length >= 2) return location.geom.coordinates;
    const loc = location as any;
    if (Array.isArray(loc?.coordinates) && loc.coordinates.length >= 2) return loc.coordinates;
    if (loc?.refPointX != null && loc?.refPointY != null) return [loc.refPointX, loc.refPointY];
    return [];
  };
  ...
  const handleEditArea = async (item: TradeAreaItem) => {
    ...
    setIsEditing(true);
    setRadiusArea(getLocationCoordinates(), radius);
        ...
        <table className="min-w-full bg-white">
          <thead className="bg-blue-100 text-blue-700">
            <tr>
              ...
              <th className="px-2 py-3 text-center text-sm font-semibold whitespace-nowrap border-r border-blue-200">
                {t('table.no')}
              </th>
            ...
            {tradeAreas.map((item, index) => {
              return (
                ...
                >
                  ...
                  <td className="px-2 py-3 text-sm text-gray-500 border-r border-gray-100 min-w-[12rem] text-center">
                    <span className="badge badge-sm px-1 py-2 text-xs font-medium bg-orange-100 text-orange-800 text-balance w-full h-full">
                      {item.currentWorkflow?.wfStatus.wfStatusName || '-'}
                    </span>
                  </td>

                  {/* 4. วันที่มีผล (ใช้ Data เดิมเป็น Placeholder) */}
